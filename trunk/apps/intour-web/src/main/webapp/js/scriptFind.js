...
function calcularRutaREST(){
 ...
 // Removes the markers from the map, but keeps them in the array.
  ...
  function mostrarInputs(){
      ...
      var encontrados= document.getElementById("CCaracteristicas");
      var auxParam;
      var auxId;
      ...
      for(var i=0; i<caracteristicas.length; i++){
      	...
      	if(caracteristicas[i].valor!=undefined){
  			...
  			boton.type='button';
  			boton.id='boton'+i;
  			boton.class='resultsCaract';
  			boton.value=caracteristicas[i].valor;
  			auxParam=caracteristicas[i].tipo;
  			auxId=boton.id;
     		boton.onclick = function () {search(auxParam,auxId)};
      		encontrados.appendChild(boton);
